...
class CMISService {
	...
	def createDocument (String folderId, CommonsMultipartFile content, String description = ''){
		if (!this.session){
			getSession()
		}
		Map<String, Object> properties = new HashMap<String, Object>();
	...
	def getDocumentPath(String documentId){
		if (!this.session){
			getSession()
		}
		Document doc = (Document) this.session.getObject(documentId)
		return doc.getPaths()[0]
		
	}
	def saveDocument(String folderId, String filename, byte[] content){
		if (!this.session){
			getSession()
		}
		Map<String, Object> properties = new HashMap<String, Object>();
		...
		ContentStream contentStream = this.session.getObjectFactory().createContentStream(filename, -1, "application/octet-stream", stream);
		Folder currentFolder = (Folder) this.session.getObject(folderId)
		// create a major version
		Document newDoc = currentFolder.createDocument(properties, contentStream, VersioningState.MAJOR);
		return newDoc.getId()
